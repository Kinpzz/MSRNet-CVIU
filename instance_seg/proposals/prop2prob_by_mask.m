...
%
function [ prob ] = prop2prob_by_mask( bbox_props, prop_mask, salient_mask )
...
for i = 1:instance_num
    ...
    x2 = bbox_props(3,i);
    y2 = bbox_props(4,i);
    %prop_region = permute(prop_mask(:,:,2,i), [2 1]);
    %prop_region = imresize(prop_region, [y2-y1+1,x2-x1+1]);
    prop_region = double(prop_mask(y1:y2, x1:x2));
    prop_region(prop_region < 0.5) = 0;
    prop_region(prop_region >= 0.5) = 1;
    bbox_map(y1:y2,x1:x2,i) = prop_region;
    bbox_count_map(y1:y2,x1:x2) = bbox_count_map(y1:y2,x1:x2) + prop_region;
end
for i = 1:instance_num
